...
public class Matrix {
    ...
    public static Matrix VectorToMatrix(Vector v) {
        int rows = v.getDimensions();
        Matrix result = new Matrix(rows, 1);  // Creating a column matrix with one column
        double[] vectorArray = v.getArray();
    
        for (int i = 0; i < rows; i++) {
            result.setValue(i, 0, vectorArray[i]);
        }
